...
*/
const { success, bold, note, error, info, wait, promptList } = require('../../../utils/console')
...
const _getAllStoppableVersions = (projectId, service) => 
	((service || {}).versions || []).filter(v => v.traffic > 0 && v.servingStatus == 'SERVING')

const _getAllStartableVersions = (projectId, service) => 
...
const _startVersions = (projectId, service, versions, options={}) => getToken(options).then(token => {
	const label = versions && (versions.length == 0 || versions.length == 1) ? 'version' : 'versions'
	const waitDone = wait(`Starting ${versions.length} ${label}...`)
	if (versions && versions.length > 0) 
 ...
 * @return {[type]}         			[description]
 */
const _startOrStopService = (service, stop, options={}) => Promise.resolve(null).then(() => {
	let projectId, startTime
	return _findService(service, options)
		.then(({ projectId: id, service }) => {
			projectId = id
			return (stop ? _getAllStoppableVersions : _getAllStartableVersions)(projectId, service, options)
		})
		.then(versions => {
			startTime = Date.now()
			return (stop ? _stopVersions : _startVersions)(projectId, service, versions, options)
		})
		.then(versions => {
			const label = (versions.length == 0 || versions.length == 1) ? 'version' : 'versions' 
			console.log(success(`Service ${bold(service)} in project ${bold(projectId)} successfully ${stop ? 'STOPPED' : 'SERVING'} in ${((Date.now() - startTime)/1000).toFixed(2)} seconds`))
