...
const TOKEN_TIMEOUT_BUFFER = 15 * 60 * 1000 // the time in ms is used to conservatively decide when to refresh the token. Instead
// of waiting for it to expire, we refresh before th expiry date to give us some leeway.
...
const getUpToDateCreds = (creds, options={ debug:false }) => {
	const { debug } = options || {}
	if ((Date.now() + TOKEN_TIMEOUT_BUFFER) < creds.expiresAt) {
		if (debug)
